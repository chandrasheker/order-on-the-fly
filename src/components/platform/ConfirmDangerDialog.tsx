...
export function ConfirmDangerDialog({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md rounded-2xl bg-app-shell border border-red-500/40 p-6 shadow-2xl"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="flex items-start gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-red-500/15 flex items-center justify-center shrink-0">
            <AlertTriangle className="w-5 h-5 text-red-800 dark:text-red-400" />
          </div>
          <div>
            <h3 id="confirm-danger-title" className="text-lg font-semibold text-foreground">
              {title}
            </h3>
        ...
        <div className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 mb-4">
          <p className="text-sm font-semibold text-red-800 dark:text-red-300">This cannot be recovered.</p>
          <p className="text-sm text-red-800/80 dark:text-red-200/80 mt-1">{details}</p>
        </div>
